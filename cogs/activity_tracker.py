...
class ActivityTracker(commands.Cog):
    ...
    async def _init_activities(self):
        ...
        for guild in self.bot.guilds:
            for member in guild.members:
                ...
                self.weekly_activity.setdefault(user_id, {})
                for act in member.activities:
                    if act.type == discord.ActivityType.custom:
                        continue
                    act_name = getattr(act, "name", str(act))
                    self.weekly_activity[user_id][act_name + "_start"] = now
                    self.activity_times[user_id].setdefault(act_name, [0, 0])  # [real, hidden]
    ...
    def load_data(self):
        ...
        try:
            with open(self.weekly_file, "r") as f:
                data = json.load(f)
                self.weekly_activity = data.get("activity_times", {})
    ...
    def save_data(self):
        try:
            ...
            with open(self.weekly_file, "w") as f:
                json.dump({"activity_times": self.weekly_activity, "voice_times": self.weekly_voice}, f, indent=4)
    ...
    async def _update_active_users_once(self):
        now = int(datetime.utcnow().timestamp())
        for user_id, activities in self.weekly_activity.items():
            ...
            if game_activities:
                real_act = max(game_activities, key=lambda a: self.activity_times[user_id][a][0])
            else:
                real_act = max(act_names, key=lambda a: self.activity_times[user_id][a][0])
    ...
    async def leaderboard_task(self):
        ...
        if now.weekday() == 6 and now.hour == 0:
            ...
            if channel:
                await self._update_active_users_once()
                await self.generate_leaderboard(channel, self.weekly_activity, self.weekly_voice, "Weekly")
                self.weekly_activity = {}
                self.weekly_voice = {}
                self.save_data()
    ...
    async def _generate_leaderboard_embeds(self, activity_data, voice_data, title_prefix=""):
        ...
        for rank, (total_time, name, top_text) in enumerate(activity_board[:limit], start=1):
            embed_activity.add_field(
                name=f"#{rank} {name} - Total: {total_time // 60} min",
        ...
        for user_id, voice_value in voice_data.items():
            ...
            voice_board.append((total_voice, user.display_name))
        ...
        for rank, (total_voice, name) in enumerate(voice_board[:limit], start=1):
            embed_voice.add_field(
                name=f"#{rank} {name}",
                value=f"Total Voice Time: {total_voice // 60} min",
                inline=False
            )
    ...
    async def generate_leaderboard(self, ctx_or_channel, activity_data, voice_data, title_prefix=""):
        ...
        if title_prefix.lower() == "weekly":
            await ctx_or_channel.send(embeds=[embed_activity, embed_voice])
        else:
            await ctx_or_channel.send(embeds=[embed_activity, embed_voice], delete_after=3600)

    @commands.command()
    async def leaderboard(self, ctx, scope: str = "alltime"):
        await self._update_active_users_once()
        if scope.lower() == "weekly":
            await self.generate_leaderboard(ctx, self.weekly_activity, self.weekly_voice, "Weekly")
        else:
    ...
    async def stats(self, ctx, member: discord.Member = None):
        ...
        embed.add_field(name="Total Voice Time", value=f"{total_voice // 60} min", inline=False)
